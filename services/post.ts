...
export async function getPosts(params?: PostParams) {
  ...
  const { limit, offset, newerThanPostId, olderThanPostId, text, likedBy, creators, tags } = params || {};
  ...
  const res = await fetch(`${envVariables.MUMBLE_API_URL}/posts?${urlParams.toString()}`,
    {
      headers: {
  ...
  const { count, data, next, previous } = (await res.json()) as ApiResponseType<Post[]>;
