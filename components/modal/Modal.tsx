...
const Modal = () => {
  const { content, isOpen, closeModal, title } = useModal();
  ...
  return (
    <div
      className={`h-modal-selector flex items-center justify-center fixed  rounded  ${positionCommon} ${transitionOptions} ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
    >
      <div
      ...
      <div className="modal-wrapper w-[calc(100%-48px)] h-[calc(100%-48px)] px-[50px] py-[50px] z-40">
        <div className="o-overlay-content overflow-hidden grow flex flex-col max-w-[600px] max-h-[100%] mx-auto">
