import { POST_ACTIONS_BAR_COMMENT_BUTTON_LABEL_SINGULAR, POST_ACTIONS_BAR_DELETE_BUTTON_LABEL, POST_ACTIONS_BAR_DELETE_BUTTON_LABEL_ACTIVE, POST_ACTIONS_BAR_DELETE_TITLE_TEXT, POST_EDITOR_PICTURE_UPLOAD_BUTTON_LABEL, POST_EDITOR_SEND_BUTTON_LABEL } from "@/utils/constants";
...
test("should create and delete a reply to a post", async ({ page }) => {
  ...
  await page.getByRole('button', { name: `${POST_ACTIONS_BAR_DELETE_BUTTON_LABEL} ${POST_ACTIONS_BAR_DELETE_BUTTON_LABEL_ACTIVE}` }).last().click();
  await page.getByRole('heading', { name: POST_ACTIONS_BAR_DELETE_TITLE_TEXT }).isVisible();
  await page.getByLabel(POST_ACTIONS_BAR_DELETE_BUTTON_LABEL).isVisible();
  await page.getByLabel(POST_ACTIONS_BAR_DELETE_BUTTON_LABEL).click();
